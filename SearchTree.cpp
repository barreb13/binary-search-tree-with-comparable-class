...
const Comparable* SearchTree::retrieve(const Comparable& comparable) const {
...
const Comparable* SearchTree::retrieveHelper(const Node* current, const Comparable& comparable) const {
	if (current == nullptr) {
		return nullptr;
	}
	else if (*current->item == comparable) {
...
void SearchTree::inOrderPrint(ostream& outStream, const Node* current) const {
	if (current != nullptr) {
		...
		outStream << *current->item << " " << current->count << endl;
